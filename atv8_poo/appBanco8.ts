...
const input = prompt();
import { Cliente, Banco, Poupanca} from "./banco8";
...
class BancoApp {
    ...
    private menu(opcoes: string): string {
        if (opcoes == '1') {
            ...
            11-   Inserir Cliente
            12-  Consultar
            13-  Atribuir titularidade de conta
            14-  Saldo total.
            15-  Trocar titularidade
            16-  Listar contas sem cliente e atribuir titulariadade
            17-  Excluir cliente
             0-  Sair`
        }
        return "";
    ...
    private cadastrarConta(): void {
        ...
        let tipo: number | undefined = getNumberInRange(Number(input("Digite 1 Para conta comum e 2 para poupança: ")),1,2);
        if (tipo == 1) {
            this.banco.inserirConta(numero);
        } else {
            let taxaDeJuros: number = Number(input("Taxa de juros: "));
            this.banco.inserirConta(numero, "poupanca", taxaDeJuros);
        }
    }
